...
namespace Microsoft.MixedReality.Toolkit.ObjectCollection
{
    ...
    [AddComponentMenu("MRTK/Object Collection/GridObjectCollection")]
    [ExecuteAlways]
            ...
            switch (SurfaceType)
            {
                ...
                case ObjectOrientationSurfaceType.Cylinder:
                    for (int i = 0; i < NodeList.Count; i++)
                    {
                        ...
                        float xAngle = (nodeGrid[i].x / circ) * Mathf.PI*2;
                        node.Transform.localPosition = new Vector3(
                            Mathf.Sin(xAngle)*radius, 
                            nodeGrid[i].y, 
                            Mathf.Cos(xAngle)*radius);
                ...
                case ObjectOrientationSurfaceType.Sphere:
                    ...
                    for (int i = 0; i < NodeList.Count; i++)
                    {
                        ...
                        float circ   = 2f * Mathf.PI * radius;
                        float xAngle =  (nodeGrid[i].x / circ) * Mathf.PI*2;
                        float yAngle = -(nodeGrid[i].y / circ) * Mathf.PI*2;
                        float sinX = Mathf.Sin(xAngle);
                        float cosX = Mathf.Cos(xAngle);
                        float sinY = Mathf.Sin(yAngle);
                        float cosY = Mathf.Cos(yAngle);
                        node.Transform.localPosition = new Vector3(
                            cosY * sinX,
                            sinY,
                            cosY * cosX ) * radius;
                ...
                case ObjectOrientationSurfaceType.Radial:
                    for (int i = 0; i < NodeList.Count; i++)
                    {
                        ...
                        int   curRow          = i/Columns + 1;
                        float radialCellAngle = radialRange / Columns;
                        float yAngle          = (radialCellAngle * (curColumn - (Columns * 0.5f)) + (radialCellAngle * .5f)) * Mathf.Deg2Rad;
                        node.Transform.localPosition = new Vector3(
                            Mathf.Sin(yAngle), 
                            0,
                            Mathf.Cos(yAngle)) * (radius / rows) * curRow;
